...
void Level3::Initialize() {
    ...
    state.map = new Map(LEVEL3_WIDTH, LEVEL3_HEIGHT, level3_data, mapTextureID, 1.0f, 17, 12);
    
    state.player = new Entity();
    state.player->entityType = PLAYER;
    state.player->position = glm::vec3(2,-6.3,0);
    state.player->movement = glm::vec3(0);
    state.player->speed = 1.85f;
    ...
    state.enemies[0].position = glm::vec3(10.5,0.1,0);
    state.enemies[0].aiType = PATROLER2;
    state.enemies[0].speed = 2.7;

    state.enemies[1].entityType = ENEMY;
    state.enemies[1].textureID = enemyTextureID;
    state.enemies[1].position = glm::vec3(0.5,0.1,0);
    state.enemies[1].aiType = PATROLER2;
    state.enemies[1].speed = 2.7;
    
    state.enemies[2].entityType = ENEMY;
    state.enemies[2].textureID = enemyTextureID;
    state.enemies[2].position = glm::vec3(5.5,-7,0);
    state.enemies[2].aiType = PATROLER2;
    state.enemies[2].speed = 3.2;
    
    
    state.goal = new Entity();
    state.goal->entityType = GOAL;
    state.goal->textureID = Util::LoadTexture("portal.png");
    state.goal->position = glm::vec3(10.5,-6.5,0);
    
    if (state.temp != NULL) {
        for (int i =0; i< LEVEL3_BUBBLE_COUNT; i++) {
            state.bubbles[i] = state.temp[i];
        }
    }
    else {
        state.bubbles = new Entity[LEVEL3_BUBBLE_COUNT];
        ...
        state.bubbles[2].position = glm::vec3(5.5,-0.3,0);
...
void Level3::Update(float deltaTime) {
    ...
    for (int i =0; i< LEVEL3_BUBBLE_COUNT; i++) {
        state.bubbles[i].Update(deltaTime, state.player, state.enemies, LEVEL3_ENEMY_COUNT, state.map);
    }
    ...
    for (int i =0; i< LEVEL3_ENEMY_COUNT; i++) {
        if (state.player->CheckCollision(&state.enemies[i])) {
            ...
            state.temp = new Entity[LEVEL3_BUBBLE_COUNT];
            for (int i =0; i< LEVEL3_BUBBLE_COUNT; i++) {
    ...
    if (state.player-> CheckCollision(state.goal) && state.numOfPoints == 8) {
        state.nextScene = 4;
    }
