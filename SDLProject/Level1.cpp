...
#define LEVEL1_WIDTH 12
#define LEVEL1_HEIGHT 8

#define LEVEL1_ENEMY_COUNT 2
#define LEVEL1_BUBBLE_COUNT 2
...
void Level1::Initialize() {
    ...
    state.enemies[0].position = glm::vec3(9.5,-0.2,0);
    state.enemies[0].aiType = PATROLER3;
    state.enemies[0].speed = 1.5;
    
    state.enemies[1].entityType = ENEMY;
    state.enemies[1].textureID = enemyTextureID;
    state.enemies[1].position = glm::vec3(9.5,-6.3,0);
    ...
    state.goal->position = glm::vec3(10.5,-3.5, 0);
    
    if (state.temp != NULL) {
        for (int i =0; i< LEVEL1_BUBBLE_COUNT; i++) {
            state.bubbles[i] = state.temp[i];
        }
    }
    else {
        state.bubbles = new Entity[LEVEL1_BUBBLE_COUNT];
        ...
        state.bubbles[0].position = glm::vec3(10.5,-0.5,0);
        
        state.bubbles[1].entityType = BUBBLE;
        state.bubbles[1].textureID = BUBBLETextureID;
        state.bubbles[1].position = glm::vec3(10.5,-6.5,0);
...
void Level1::Update(float deltaTime) {
    ...
    for (int i =0; i< LEVEL1_BUBBLE_COUNT; i++) {
        state.bubbles[i].Update(deltaTime, state.player, state.enemies, LEVEL1_ENEMY_COUNT, state.map);
    }
    ...
    for (int i =0; i< LEVEL1_ENEMY_COUNT; i++) {
        if (state.player->CheckCollision(&state.enemies[i])) {
            ...
            state.temp = new Entity[LEVEL1_BUBBLE_COUNT];
            for (int i =0; i< LEVEL1_BUBBLE_COUNT; i++) {
    ...
    if (state.player-> CheckCollision(state.goal) && state.numOfPoints == 2) {
        state.nextScene = 2;
    }
