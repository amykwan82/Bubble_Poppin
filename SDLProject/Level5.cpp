...
#define LEVEL5_WIDTH 12
#define LEVEL5_HEIGHT 8

#define LEVEL5_ENEMY_COUNT 5
#define LEVEL5_BUBBLE_COUNT 5
...
void Level5::Initialize() {
    ...
    state.map = new Map(LEVEL5_WIDTH, LEVEL5_HEIGHT, level5_data, mapTextureID, 1.0f, 17, 12);
    
    state.player = new Entity();
    state.player->entityType = PLAYER;
    state.player->position = glm::vec3(0,-6.5,0);
    state.player->movement = glm::vec3(0);
    state.player->speed = 2.10f;
    ...
    state.enemies[0].position = glm::vec3(-0.1,-0.1,0);
    state.enemies[0].aiType = PATROLER5;
    state.enemies[0].speed = 7;
    
    state.enemies[1].entityType = ENEMY;
    state.enemies[1].textureID = enemyTextureID;
    state.enemies[1].position = glm::vec3(11.1,-6.5,0);
    state.enemies[1].aiType = PATROLER5;
    state.enemies[1].speed = 3.8;
    
    state.enemies[2].entityType = ENEMY;
    state.enemies[2].textureID = enemyTextureID;
    state.enemies[2].position = glm::vec3(5.4,-3,0);
    state.enemies[2].aiType = PATROLER6;
    state.enemies[2].speed = 2.5;
    
    state.enemies[3].entityType = ENEMY;
    state.enemies[3].textureID = enemyTextureID;
    state.enemies[3].position = glm::vec3(-0.4,-3,0);
    state.enemies[3].aiType = PATROLER7;
    state.enemies[3].speed = 2.5;
    
    state.enemies[4].entityType = ENEMY;
    state.enemies[4].textureID = enemyTextureID;
    state.enemies[4].position = glm::vec3(5.5,0.1,0);
    state.enemies[4].aiType = PATROLER8;
    state.enemies[4].speed = 2.5;
    
    state.goal = new Entity();
    state.goal->entityType = GOAL;
    state.goal->textureID = Util::LoadTexture("portal.png");
    state.goal->position = glm::vec3(5.5,-3,0);
    
    if (state.temp != NULL) {
        for (int i =0; i< LEVEL5_BUBBLE_COUNT; i++) {
            state.bubbles[i] = state.temp[i];
        }
    }
    else {
        state.bubbles = new Entity[LEVEL5_BUBBLE_COUNT];
...
void Level5::Update(float deltaTime) {
    ...
    state.goal->Update(deltaTime, state.player, state.enemies, LEVEL5_ENEMY_COUNT, state.map);
    ...
    for (int i =0; i< LEVEL5_BUBBLE_COUNT; i++) {
        state.bubbles[i].Update(deltaTime, state.player, state.enemies, LEVEL5_ENEMY_COUNT, state.map);
    }
    ...
    for (int i =0; i< LEVEL5_ENEMY_COUNT; i++) {
        if (state.player->CheckCollision(&state.enemies[i])) {
            ...
            state.temp = new Entity[LEVEL5_BUBBLE_COUNT];
            for (int i =0; i< LEVEL5_BUBBLE_COUNT; i++) {
    ...
    if (state.player-> CheckCollision(state.goal) && state.numOfPoints == 18) {
        state.nextScene = 7;
    }
